...
问 AI「这个方案有人申请过吗」，它答得头头是道，专利号却是编的；找个免费的查中国专利，权利要求和法律状态全是空的；想买正经数据库，报价一年十几万。

这个 Skill 把一个真实的全球专利库接到 AI 手里——**查得到就是查得到，查不到就说查不到**，每一条都有公开号可以去官方库核对。

数据来自 iprdb 全球专利库，覆盖中国、美国、欧洲、日本、韩国和 WIPO，**中国专利收录最完整**——权利要求、法律事件流水、摘要附图都有，不是只给个标题就完事。

## 两条线

**检索 · 一次 ¥0.10**

关键词或布尔式找专利；给一个公开号返回著录项目、权利要求、法律状态；用语义相似度找技术方案最接近的专利；查引证关系——引了谁、被谁引、引了哪些论文标准。

**查新 · 一次约 ¥15，出 Word 报告**

丢一段技术方案描述进去，异步跑几分钟，返回对比文件、相似度分析和查新结论，**并能直接导出 DOCX**——立项材料、申请前自查可以直接用。
...
新用户 **¥50 体验额度**，按检索一次一毛钱算够跑 **500 次**，不用绑卡、不用先充值。

也可以先白嫖：申请一把 `pm_test_` 测试密钥，检索类每天 30 次，查新返回模拟结果，**全程不计费**。参数拼对了再换生产密钥。

密钥在 [api.ip930.com/features/api-platform](https://api.ip930.com/features/api-platform) 创建，一分钟的事。配置见 [INSTALL.md](INSTALL.md)。

**不依赖 MCP，只用 Python 标准库**，不需要 `pip install`——任何能跑 Bash 的 AI 客户端都能用。
